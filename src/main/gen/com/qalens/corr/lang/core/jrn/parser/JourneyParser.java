...
package com.qalens.corr.lang.core.jrn.parser;
...
public class JourneyParser implements PsiParser, LightPsiParser {
  ...
  public static final TokenSet[] EXTENDS_SETS_ = new TokenSet[] {
    create_token_set_(ASSIGNMENT_STEP, IF_ELSE_STEP, JOURNEY_STEP, LISTENER_STEP,
      PRINT_STEP, REST_STEP, SEND_DATA_OVER_WEB_SOCKET_CLIENT_STEP, STEP,
      SYNC_STEP, TRANSACTION_STEP, UNDEF_STEP, VARIABLE_ACTION_STEP,
      WAIT_STEP, WEBSOCKET_CLIENT_CONNECT_STEP),
  };
  ...
  /* ********************************************************** */
  // (VariableReference '.' ('round' |'unique_random_elements'| 'random' | 'sub' | 'div' | 'mod' | 'left' | 'right' | 'array' | 'contains' | 'indexOf' | 'chunked') '(' Expression ')') | (('round' | 'random'|'unique_random_elements' | 'chunked' |'sub' | 'div' | 'mod' | 'left' | 'right' | 'array' | 'contains' | 'indexOf') '(' (Expression ',' Expression)  ')')
  ...
  // VariableReference '.' ('round' |'unique_random_elements'| 'random' | 'sub' | 'div' | 'mod' | 'left' | 'right' | 'array' | 'contains' | 'indexOf' | 'chunked') '(' Expression ')'
  ...
  // 'round' |'unique_random_elements'| 'random' | 'sub' | 'div' | 'mod' | 'left' | 'right' | 'array' | 'contains' | 'indexOf' | 'chunked'
  private static boolean BinaryFunction_0_2(PsiBuilder b, int l) {
    ...
    if (!r) r = consumeToken(b, CHUNKED);
    return r;
  }

  // ('round' | 'random'|'unique_random_elements' | 'chunked' |'sub' | 'div' | 'mod' | 'left' | 'right' | 'array' | 'contains' | 'indexOf') '(' (Expression ',' Expression)  ')'
  ...
  // 'round' | 'random'|'unique_random_elements' | 'chunked' |'sub' | 'div' | 'mod' | 'left' | 'right' | 'array' | 'contains' | 'indexOf'
  ...
  /* ********************************************************** */
  // 'send' ('binary')? Expression  'on' 'websocket' 'named' Expression
  public static boolean SendDataOverWebSocketClientStep(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "SendDataOverWebSocketClientStep")) return false;
    if (!nextTokenIs(b, SEND)) return false;
    boolean r, p;
    Marker m = enter_section_(b, l, _NONE_, SEND_DATA_OVER_WEB_SOCKET_CLIENT_STEP, null);
    r = consumeToken(b, SEND);
    p = r; // pin = 1
    r = r && report_error_(b, SendDataOverWebSocketClientStep_1(b, l + 1));
    r = p && report_error_(b, Expression(b, l + 1)) && r;
    r = p && report_error_(b, consumeTokens(b, -1, ON, WEBSOCKET, NAMED)) && r;
    r = p && Expression(b, l + 1) && r;
    exit_section_(b, l, m, r, p, null);
    return r || p;
  }
  ...
  // ('binary')?
  private static boolean SendDataOverWebSocketClientStep_1(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "SendDataOverWebSocketClientStep_1")) return false;
    consumeToken(b, BINARY);
    return true;
  }
  ...
  /* ********************************************************** */
  // UndefStep | PrintStep | TransactionStep | WaitStep | WebsocketClientConnectStep |SendDataOverWebSocketClientStep  | AssignmentStep | SyncStep | ListenerStep | IfElseStep  | RestStep | VariableActionStep | JourneyStep
  public static boolean Step(PsiBuilder b, int l) {
    ...
    r = UndefStep(b, l + 1);
    if (!r) r = PrintStep(b, l + 1);
    if (!r) r = TransactionStep(b, l + 1);
    if (!r) r = WaitStep(b, l + 1);
    if (!r) r = WebsocketClientConnectStep(b, l + 1);
    if (!r) r = SendDataOverWebSocketClientStep(b, l + 1);
  ...
  /* ********************************************************** */
  // 'undef' | 'measure'| 'send' | 'connect' |'websocket'| 'let' | 'sync' | 'listen' | 'if' | 'async' | 'print' | '}' | 'respond' | 'call' | 'wait' | RestVerb | identifier
  static boolean StepStart(PsiBuilder b, int l) {
    ...
    if (!r) r = consumeToken(b, WEBSOCKET);
  ...
  /* ********************************************************** */
  // 'measure' Expression Block
  public static boolean TransactionStep(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "TransactionStep")) return false;
    if (!nextTokenIs(b, MEASURE)) return false;
    boolean r, p;
    Marker m = enter_section_(b, l, _NONE_, TRANSACTION_STEP, null);
  ...
  /* ********************************************************** */
  // (VariableReference '.' ('fake' | 'from_json' | 'read_wav' | 'random_element' | 'encode' | 'now' | 'len' ) '('')')|(('fake' | 'read_wav'|'from_json' | 'random_element' | 'encode' | 'now' | 'len' ) '(' Expression  ')')
  ...
  // VariableReference '.' ('fake' | 'from_json' | 'read_wav' | 'random_element' | 'encode' | 'now' | 'len' ) '('')'
  ...
  // 'fake' | 'from_json' | 'read_wav' | 'random_element' | 'encode' | 'now' | 'len'
  ...
  // ('fake' | 'read_wav'|'from_json' | 'random_element' | 'encode' | 'now' | 'len' ) '(' Expression  ')'
  ...
  // 'fake' | 'read_wav'|'from_json' | 'random_element' | 'encode' | 'now' | 'len'
  ...
  /* ********************************************************** */
  // 'undef' VariableReference
  public static boolean UndefStep(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "UndefStep")) return false;
    if (!nextTokenIs(b, UNDEF)) return false;
    boolean r, p;
    Marker m = enter_section_(b, l, _NONE_, UNDEF_STEP, null);
    r = consumeToken(b, UNDEF);
    p = r; // pin = 1
    r = r && VariableReference(b, l + 1);
    exit_section_(b, l, m, r, p, null);
  ...
  /* ********************************************************** */
  // 'connect' 'websocket' 'named' Expression 'with' 'url' Expression 'and' 'listener' VariableReference '=>' Block
  public static boolean WebsocketClientConnectStep(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "WebsocketClientConnectStep")) return false;
    if (!nextTokenIs(b, CONNECT)) return false;
    boolean r, p;
    Marker m = enter_section_(b, l, _NONE_, WEBSOCKET_CLIENT_CONNECT_STEP, null);
    r = consumeTokens(b, 1, CONNECT, WEBSOCKET, NAMED);
    p = r; // pin = 1
    r = r && report_error_(b, Expression(b, l + 1));
    r = p && report_error_(b, consumeTokens(b, -1, WITH, URL)) && r;
    r = p && report_error_(b, Expression(b, l + 1)) && r;
    r = p && report_error_(b, consumeTokens(b, -1, AND, LISTENER)) && r;
    r = p && report_error_(b, VariableReference(b, l + 1)) && r;
    r = p && report_error_(b, consumeToken(b, FATARROW)) && r;
    r = p && Block(b, l + 1) && r;
    exit_section_(b, l, m, r, p, null);
    return r || p;
  }
